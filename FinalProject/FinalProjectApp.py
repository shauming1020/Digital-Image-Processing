...
from preprocessing import cv2, clahe_hist 
...
class AppWindow(QDialog):
    ...
    def selectModel(self):
        ...
        if self.loaded_model:
            ...
            ## Find Contours
            _, contours_p2, hierarchy_p = cv2.findContours(np.asarray(self.predict),\
            ...
            contours_p = []
            for i in range(len(contours_p2)):
                area_p = cv2.contourArea(contours_p2[i])
                if area_p >= 1000:
                    contours_p.append(contours_p2[i])
